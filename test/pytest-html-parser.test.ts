...
describe('extractPytestJSON', () => {
  describe('Generated fixtures', () => {
    it('extracts JSON from generated pytest-html report', () => {
      const htmlPath = join(FIXTURES_DIR, 'generated/python/pytest-report.html');
      ...
      const passTest = report!.tests.find((t) => t.nodeid.includes('test_passes_basic_assertion'));
      expect(passTest).toBeDefined();
      expect(passTest!.outcome).toBe('passed');

      const failTest = report!.tests.find((t) => t.nodeid.includes('test_fails_deliberately'));
      ...
      const skipTest = report!.tests.find((t) => t.nodeid.includes('test_skipped'));
