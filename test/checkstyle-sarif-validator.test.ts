...
describe('Checkstyle SARIF validator', () => {
  const sarifPath = join(FIXTURES_DIR, 'checkstyle-result.sarif');

  it('validates real checkstyle SARIF output', () => {
    const content = readFileSync(sarifPath, 'utf-8');
    const result = validateCheckstyleSARIF(content);

    expect(result.valid).toBe(true);
  });

  it('rejects invalid JSON', () => {
    const result = validateCheckstyleSARIF('not valid json {]');
    expect(result.valid).toBe(false);
    expect(result.error).toContain('Invalid JSON');
  });

  it('rejects SARIF without version field', () => {
    const sarif = {
      runs: [
        {
          tool: {
  ...
  it('rejects SARIF without runs array', () => {
    const sarif = {
      version: '2.1.0',
    ...
    const result = validateCheckstyleSARIF(JSON.stringify(sarif));
    expect(result.valid).toBe(false);
    expect(result.error).toContain('runs');
  });

  it('rejects SARIF with empty runs array but no results', () => {
    const sarif = {
      version: '2.1.0',
      runs: [
        {
