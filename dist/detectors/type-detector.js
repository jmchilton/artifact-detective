...
function detectByContent(content, format) {
...
function detectHtmlType(content, lowerContent) {
...
function detectJsonType(content, lowerContent) {
    try {
        ...
        for (const line of lines) {
            ...
            try {
                ...
                if (obj.reason &&
                    ["compiler-message", "compiler-artifact", "build-finished"].includes(obj.reason)) {
                    return "clippy-json";
                }
            }
            catch {
        ...
        if (data.config && data.suites && Array.isArray(data.suites)) {
            ...
            if (data.config.rootDir !== undefined ||
...
function detectXmlType(content, lowerContent) {
    // JUnit XML format
    if (lowerContent.includes("<testsuite") ||
        lowerContent.includes("<testsuites")) {
        return "junit-xml";
    }
    return "unknown";
}
function detectTxtType(content, lowerContent) {
