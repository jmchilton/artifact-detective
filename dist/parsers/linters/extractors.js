...
export function detectLinterType(jobName, logContent) {
...
export function extractLinterOutput(linterType, logContent) {
...
function extractESLintOutput(lines) {
    // If this looks like raw ESLint output (not embedded in logs), return as-is
    const hasEslintPattern = lines.some((line) => /\d+:\d+\s+(error|warning)/.test(line) || /\d+\s+problem/.test(line));
    if (hasEslintPattern) {
        return lines.join("\n").trim();
    }
    ...
    for (const line of lines) {
        ...
        if (line.match(/eslint.*\.(js|ts|jsx|tsx)/i) ||
...
function extractPrettierOutput(lines) {
    const outputLines = [];
    let inOutput = false;
    for (const line of lines) {
...
function extractPythonLinterOutput(lines, linterType) {
...
function extractTSCOutput(lines) {
...
function extractFormatterOutput(lines, formatterType) {
    const outputLines = [];
    let inOutput = false;
    for (const line of lines) {
        ...
        if (inOutput) {
            ...
            if (line.match(/would reformat/i) ||
...
function extractMypyOutput(lines) {
    // If this looks like raw mypy output (not embedded in logs), return as-is
    const hasMypyPattern = lines.some((line) => /^[a-zA-Z0-9_\-\/\.]+\.py:\d+:\s*(error|warning|note):/.test(line));
    if (hasMypyPattern) {
        return lines.join("\n").trim();
    }
    // Otherwise, extract from CI logs
...
function extractClippyOutput(lines) {
    // If this looks like raw clippy output (not embedded in logs), return as-is
    const hasClippyPattern = lines.some((line) => /^(warning|error):\s/.test(line) || /-->\s+\S+\.rs:\d+:\d+/.test(line));
    if (hasClippyPattern) {
        return lines.join("\n").trim();
    }
    // Otherwise, extract from CI logs
